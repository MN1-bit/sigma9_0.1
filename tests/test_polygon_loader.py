# ============================================================================
# Polygon Loader Tests
# ============================================================================
# 📌 이 파일의 역할:
#   - polygon_loader.py 모듈의 단위 테스트
...
from backend.data.polygon_loader import PolygonLoader, US_HOLIDAYS
...
class TestTradingDayCalculation:
    ...
    def test_weekday_is_trading_day(self):
        ...
        assert PolygonLoader.is_trading_day(monday) is True
        
        # 2024-12-17은 화요일
        tuesday = datetime(2024, 12, 17)
        assert PolygonLoader.is_trading_day(tuesday) is True
    
    def test_weekend_is_not_trading_day(self):
        """주말은 거래일 아님"""
        # 2024-12-14는 토요일
        saturday = datetime(2024, 12, 14)
        assert PolygonLoader.is_trading_day(saturday) is False
        
        # 2024-12-15는 일요일
        sunday = datetime(2024, 12, 15)
        assert PolygonLoader.is_trading_day(sunday) is False
    
    def test_holiday_is_not_trading_day(self):
        """공휴일은 거래일 아님"""
        # 2024-12-25은 크리스마스
        christmas = datetime(2024, 12, 25)
        assert PolygonLoader.is_trading_day(christmas) is False
        
        # 2024-07-04은 독립기념일
        july4 = datetime(2024, 7, 4)
        assert PolygonLoader.is_trading_day(july4) is False
    ...
    def test_get_trading_days_between(self):
        ...
        trading_days = PolygonLoader.get_trading_days_between(start, end)
    ...
    def test_get_last_trading_day(self):
        """가장 최근 거래일 반환"""
        last_day = PolygonLoader.get_last_trading_day()
...
class TestIncrementalUpdate:
    ...
    async def test_update_calls_api_for_missing_days(self):
        ...
        mock_client.fetch_grouped_daily.return_value = [
            {"ticker": "AAPL", "date": "2024-12-16", "open": 150, "high": 152, 
             "low": 149, "close": 151, "volume": 50000000}
        ]
        
        loader = PolygonLoader(mock_db, mock_client)
        
        # 현재 시간을 2024-12-17 화요일로 가정하고 테스트
        with patch.object(PolygonLoader, 'get_last_trading_day', return_value="2024-12-16"):
