...
class PyQtGraphChartWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.price_plot = self.graphics_layout.addPlot(
            row=0, col=0, axisItems={"bottom": self.date_axis}
        )
        ...
        self.volume_plot = self.graphics_layout.addPlot(
            row=1, col=0, axisItems={"bottom": self.volume_date_axis}
        )
    ...
    def _format_volume_axis(self):
        ...
        axis.tickStrings = lambda values, scale, spacing: [
            format_volume(v) for v in values
        ]
    ...
    def _setup_tooltips(self):
        ...
        self._price_tooltip = pg.TextItem(
            text="", color="white", fill=pg.mkBrush(0, 0, 0, 180), anchor=(0, 1)
        )
    ...
    def set_volume_data(self, volume_data: List[Dict]):
        ...
        for i, v in enumerate(volume_data):
            ...
            self._volume_data.append({"index": idx, "time": t, "volume": v["volume"]})
    ...
    def set_ma_data(
        ...
        if times:
            line = self.price_plot.plot(
                times, values, pen=pg.mkPen(color, width=1), name=f"MA{period}"
            )
    ...
    def set_price_levels(
        self, entry: float = None, stop_loss: float = None, take_profit: float = None
    ):
        """
        ...
        # ─────────────────────────────────────────────────────────────────
            ...
            if self._candle_item:
                self._candle_item.add_bar(
                    new_idx, bar["open"], bar["high"], bar["low"], bar["close"]
                )
    ...
    def update_current_candle(self, price: float, volume: int = 0):
        ...
        if self._candle_item:
            last_idx = last["index"]
            self._candle_item.update_bar(
                last_idx, last["open"], last["high"], last["low"], last["close"]
            )
...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        test_volumes.append(
            {"time": base_time + i * 86400, "volume": vol, "is_up": is_up}
        )
        price = c
    ...
    # Ignition 마커 테스트
    chart.add_ignition_marker(
        test_candles[50]["time"], test_candles[50]["high"], score=85
    )
