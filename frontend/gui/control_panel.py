...
class StatusIndicator(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.text.setStyleSheet(
            f"color: {theme.get_color('text_secondary')}; background: transparent; border: none; font-size: 11px;"
        )
...
class LoadingOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.label.setStyleSheet(
            f"color: {theme.get_color('primary')}; font-size: 14px; font-weight: bold;"
        )
...
class ControlPanel(QFrame):
    ...
    def _init_ui(self):
        ...
        self.connect_btn = self._create_button(
            "🔌 Connect", "primary", self._on_connect_clicked
        )
        self.disconnect_btn = self._create_button(
            "⏏️ Disconnect", "text_secondary", self._on_disconnect_clicked
        )
        ...
        self.start_btn = self._create_button(
            "🚀 Start Engine", "success", self.start_clicked.emit
        )
        self.stop_btn = self._create_button(
            "⏹ Stop Engine", "warning", self.stop_clicked.emit
        )
        ...
        # 7. Time Display [08-001]
        self.time_display = TimeDisplayWidget(theme=theme)
        ...
        self.settings_btn.clicked.connect(
            lambda: print("[DEBUG] ControlPanel: Settings Button Clicked!")
        )
    ...
    def _init_strategy_ui(self, layout):
        ...
        label.setStyleSheet(
            f"color: {theme.get_color('text_secondary')}; font-size: 11px; border: none; background: transparent;"
        )
    ...
    def _create_separator(self):
        lbl = QLabel("|")
        lbl.setStyleSheet(
            f"color: {theme.get_color('border')}; border: none; background: transparent;"
        )
        return lbl
