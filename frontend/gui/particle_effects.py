...
class ConstellationEffect(BackgroundEffect):
    ...
    def draw(self, painter: QPainter):
        ...
        for p in self.particles:
            painter.setBrush(
                QColor(p.color[0], p.color[1], p.color[2], int(150 * p.life))
            )
...
class DigitalDustEffect(BackgroundEffect):
    ...
    def _init_particles(self):
        for _ in range(100):
            color = (
                (255, 215, 0) if random.random() > 0.5 else (192, 192, 192)
            )  # Gold or Silver
            self.particles.append(
                Particle(
                    x=random.uniform(0, self.width),
...
class BokehEffect(BackgroundEffect):
    ...
    def update(self):
        for p in self.particles:
            ...
            # Loop around
            if p.x > self.width + 150:
                p.x = -150
            if p.y > self.height + 150:
                p.y = -150
            if p.y < -150:
                p.y = self.height + 150
    ...
    def draw(self, painter: QPainter):
        ...
        for p in self.particles:
            ...
            grad.setColorAt(0, c)
            grad.setColorAt(
                0.5, QColor(p.color[0], p.color[1], p.color[2], int(alpha * 0.4))
            )
...
class GoldenRainEffect(BackgroundEffect):
    ...
    def draw(self, painter: QPainter):
        ...
        for p in self.particles:
            ...
            grad.setColorAt(0, c)
            grad.setColorAt(
                0.5, QColor(p.color[0], p.color[1], p.color[2], int(alpha * 0.5))
            )
            grad.setColorAt(1, QColor(0, 0, 0, 0))
            painter.setBrush(QBrush(grad))
            painter.drawEllipse(QPointF(p.x, p.y), p.size * 2, p.size * 2)
...
class RisingBubblesEffect(BackgroundEffect):
    ...
    def draw(self, painter: QPainter):
        ...
        for p in self.particles:
            ...
            grad.setColorAt(
                1, QColor(p.color[0], p.color[1], p.color[2], int(alpha * 0.3))
            )
            painter.setBrush(QBrush(grad))
            painter.drawEllipse(QPointF(p.x, p.y), p.size, p.size)
    ...
    def paintEvent(self, event):
        ...
        for p in self.active_particles:
            ...
            if p.is_image and self.coin_pixmap:
                ...
                color = QColor(
                    p.color[0], p.color[1], p.color[2], int(final_alpha * 255)
                )
        ...
        for _ in range(50):
            ...
            self.active_particles.append(
                Particle(
                    ...
                    color=random.choice(
                        [(244, 67, 54), (229, 57, 53), (239, 154, 154)]
                    ),
                    decay=0.012,
                    size_decay=0.05,
                )
