...
class RestAdapter:
    ...
    async def control_engine(self, command: str) -> Dict[str, Any]:
        ...
        try:
            client = await self._get_client()
            response = await client.post("/api/control", json={"command": command})
    ...
    async def run_scanner(self, strategy_name: str = "seismograph") -> Dict[str, Any]:
        ...
        try:
            ...
            response = await client.post(
                "/api/scanner/run", params={"strategy_name": strategy_name}
            )
    ...
    async def promote_to_tier2(self, tickers: List[str]) -> Dict[str, Any]:
        ...
        try:
            client = await self._get_client()
            response = await client.post(
                "/api/tier2/promote", json={"tickers": tickers}
            )
    ...
    async def demote_from_tier2(self, tickers: List[str]) -> Dict[str, Any]:
        ...
        try:
            client = await self._get_client()
            response = await client.post("/api/tier2/demote", json={"tickers": tickers})
