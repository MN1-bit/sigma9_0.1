...
class BackendClient(QObject):
    ...
    ignition_updated = pyqtSignal(
        dict
    )  # {"ticker": str, "score": float, "passed_filter": bool}
    bar_received = pyqtSignal(
        dict
    )  # Phase 4.A.0: {"ticker": str, "timeframe": str, "bar": dict}
    tick_received = pyqtSignal(
        dict
    )  # Phase 4.A.0.b: {"ticker": str, "price": float, "volume": int}
    ...
    async def connect(self) -> bool:
        ...
        if (
            ...
            or self.state == ConnectionState.RUNNING
        ):
            return True
        ...
        try:
            ...
            if watchlist_data:
                ...
                self.log_message.emit(
                    f"📋 Watchlist loaded: {len(watchlist_data)} items"
                )
    ...
    async def refresh_watchlist(self):
        ...
        if watchlist_data:
            ...
            self.watchlist_updated.emit(watchlist_data)
            self.log_message.emit(
                f"📋 Watchlist refreshed: {len(watchlist_data)} items"
            )
