...
#
# 📖 [03-001] Archive 보관됨: 2026-01-08
#   원본: backend/strategies/seismograph.py
#   위치: docs/archive/seismograph_backup.py
# ============================================================================
...
# backend 폴더를 경로에 추가 (archive에서 실행 시)
...
from backend.core.technical_analysis import DynamicStopLoss
from backend.core.interfaces.scoring import ScoringStrategy

# V3 설정 임포트
from backend.strategies.score_v3_config import (
    V3_WEIGHTS,
    ZSCORE_SIGMOID,
    SUPPORT_CONFIG,
    ACCUMBAR_CONFIG,
    SIGNAL_MODIFIER_CONFIG,
    PERCENTILE_CONFIG,
    REDUNDANCY_PENALTY_CONFIG,
)
...
class SeismographStrategy(StrategyBase, ScoringStrategy):
    ...
    def calculate_watchlist_score_v3(
        self, ticker: str, daily_data: Any, current_vwap: Optional[float] = None
    ) -> float:
        """
        ...
        try:
            ...
            # V3 강도 계산
            intensities = self._calculate_signal_intensities_v3(
                daily_data, current_vwap
            )
    ...
    def _calculate_signal_intensities_v3(
        self, data: Any, current_vwap: Optional[float] = None
    ) -> Dict[str, float]:
        """
        V3 신호 강도 계산 (0.0 ~ 1.0)
        ...
        return {
            ...
            "obv_divergence": self._calc_obv_divergence_intensity_v3(
                data
            ),  # V3: Z-Score 표준화
            "accumulation_bar": self._calc_accumulation_bar_intensity_v3(
                data
            ),  # V3: 로그 스케일
    ...
    def _calc_volume_dryout_intensity_v3(self, data: Any) -> float:
        ...
        try:
            ...
            support_dist = (support_factor - min_loc) / (
                1.0 - min_loc
            )  # -0.67 ~ 1.0 범위
    ...
    def _calc_accumulation_bar_intensity_v3(
        self, data: Any, float_shares: int = 10_000_000
    ) -> float:
        """
        ...
        try:
            ...
            accum_vols = [float(d.get("volume", d.get("Volume", 0))) for d in period]
    ...
    def calculate_watchlist_score_detailed(
        self, ticker: str, daily_data: Any
    ) -> Dict[str, Any]:
        """
    ...
    def _check_obv_divergence(self, data: Any) -> float:
        ...
        try:
            ...
            price_slope = (
                (closes[-1] - closes[0]) / len(closes) if len(closes) > 1 else 0
            )
    ...
    def calculate_trigger_score(
        self, ticker: str, tick_data: Any = None, bar_data: Any = None
    ) -> float:
        """
    ...
    def _calculate_tick_velocity(self, ticker: str) -> float:
        ...
        try:
            ...
            # 최근 10초 틱 수
            ticks_10s = sum(
                1 for t in ticks if (now - t.timestamp).total_seconds() <= 10
            )

            # 최근 60초 틱 수 (1분 평균)
            ticks_60s = sum(
                1 for t in ticks if (now - t.timestamp).total_seconds() <= 60
            )
            ...
            # 부분 점수 (절반 이상이면 0.5점)
            if avg_ticks_per_10s > 0 and ticks_10s > avg_ticks_per_10s * (
                multiplier / 2
            ):
                return 0.5
    ...
    def check_anti_trap_filter(
        self, ticker: str, price: float, bid: float, ask: float, timestamp: datetime
    ) -> Tuple[bool, str]:
        """
        Anti-Trap 필터 검증
    ...
    def load_watchlist_context(self, watchlist: List[Dict[str, Any]]) -> None:
        """
        ...
        print(
            f"[{self.name}] Watchlist Context 로드 완료: {len(self._watchlist)}개 티커"
        )
        ...
        tradeable = sum(
            1 for ctx in self._watchlist_context.values() if ctx.get("can_trade")
        )
    ...
    def on_tick(
        ...
        # === 틱 저장 ===
        tick = TickData(price=price, volume=volume, timestamp=timestamp, side=side)
        ...
        while (
            ...
            and self._tick_buffer[ticker][0].timestamp < cutoff
        ):
        ...
        print(
            f"[{self.name}] 🔥 BUY Signal: {ticker} @ ${price:.2f} "
...
if __name__ == "__main__":
    ...
    mock_data = pd.DataFrame(
        {
            ...
            "volume": [100000] * 27
            + [500000, 50000, 30000],  # 마지막에 스파이크 + 드라이아웃
        },
        index=dates,
    )
    ...
    print("\n✓ 설정 변경 테스트:")
    print(
        f"  변경 전 accumulation_threshold: {strategy.config['accumulation_threshold']['value']}"
    )
    strategy.set_config({"accumulation_threshold": {"value": 55}})
    print(
        f"  변경 후 accumulation_threshold: {strategy.config['accumulation_threshold']['value']}"
    )
    ...
    print("  - 1분봉 버퍼: 마지막 거래량 80,000 (5분 평균의 ~7배)")
