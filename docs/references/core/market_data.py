...
class MarketDataManager(QThread):
    ...
    def _get_data_count(self, symbol: str) -> int:
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT COUNT(*) FROM historical_prices WHERE symbol = ?", (symbol,)
        )
    ...
    def _get_last_date(self, symbol: str) -> Optional[str]:
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT MAX(date) FROM historical_prices WHERE symbol = ?", (symbol,)
        )
    ...
    def get_historical_prices(self, symbol: str, days: int = 252) -> pd.DataFrame:
        ...
        df = pd.read_sql_query(
            query, self.conn, params=(symbol, days), parse_dates=["date"]
        )
    ...
    def _refresh_cache_if_needed(self) -> bool:
        ...
        try:
            ...
            self.log_message.emit(
                f"📊 일봉 통계 캐시 갱신: Mean={self._cached_mean:.2f}, Std={self._cached_std:.2f}"
            )
            return True
    ...
    def _get_last_intraday_datetime(self, symbol: str) -> Optional[str]:
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT MAX(datetime) FROM intraday_prices WHERE symbol = ?", (symbol,)
        )
        result = cursor.fetchone()[0]
        return result
    ...
    def _get_intraday_count(self, symbol: str) -> int:
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT COUNT(*) FROM intraday_prices WHERE symbol = ?", (symbol,)
        )
        return cursor.fetchone()[0]
