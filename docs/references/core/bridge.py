...
class IBKRBridge(QThread):
    ...
    def subscribe_market_data(
        self, symbols: List[str], outside_rth: bool = True
    ) -> None:
        """
        실시간 시세 구독
        ...
        try:
            ...
            if self._bridge_tick_count % 50 == 0:
                self.log_message.emit(
                    f"🔔 Bridge 가격 수신: {symbol} (틱 #{self._bridge_tick_count})"
                )
    ...
    def subscribe_vix_futures(self) -> None:
        ...
        try:
            ...
            # VX 선물 계약 생성
            vx_front = Future(
                "VX", exchange="CFE", lastTradeDateOrContractMonth=front_contract_month
            )
            vx_back = Future(
                "VX", exchange="CFE", lastTradeDateOrContractMonth=back_contract_month
            )
            ...
            self.log_message.emit(
                f"📡 VIX 선물 구독: VX {front_contract_month} (근월), VX {back_contract_month} (원월)"
            )
