...
class UniverseSelector(QThread):
    ...
    def calculate_relative_strength(self, symbol: str) -> float:
        ...
        try:
            ...
            # 1개월 전 가격 (약 21 거래일)
            price_1m = (
                hist["Close"].iloc[-21] if len(hist) >= 21 else hist["Close"].iloc[0]
            )

            # 3개월 전 가격 (약 63 거래일)
            price_3m = (
                hist["Close"].iloc[-63] if len(hist) >= 63 else hist["Close"].iloc[0]
            )
    ...
    def get_target_etf(self) -> str:
        ...
        if self._scores:
            ...
            self.log_message.emit(
                f"🎯 선정된 ETF: {self._target_etf} ({self._scores[self._target_etf]:.2f}%)"
            )
        else:
    ...
    def scan_growth_stocks(self) -> List[str]:
        ...
        for symbol in self.SCAN_UNIVERSE:
            try:
                ...
                self.log_message.emit(
                    f"  ✅ {symbol} 통과 (가격: ${current_price:.2f})"
                )
