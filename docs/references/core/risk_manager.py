...
class RiskManager(QObject):
    ...
    def check_kill_switch(
        ...
        if vix_1m > vix_3m:
            ...
            self.log_message.emit(
                f"🚨 킬 스위치: HALT_ALL - VIX 백워데이션 (1M:{vix_1m:.2f} > 3M:{vix_3m:.2f})"
            )
        ...
        if tnx_change > 0.05:  # 5% 이상 변화
            ...
            self.log_message.emit(
                f"⚠️ 킬 스위치: HALT_LONG - 금리 급등 ({tnx_change * 100:.1f}%)"
            )
    ...
    def calculate_position_size(
        self, account: float, price: float, yang_zhang_vol: float
    ) -> int:
        """
    ...
    def apply_volatility_targeting(
        self, current_volatility: float, target_vol: float = None
    ) -> float:
        """
        변동성 타겟팅
    ...
    def approve_order(
        self, kill_status: str, daily_loss: float, account: float
    ) -> bool:
        """
        ...
        if account > 0:
            ...
            if loss_ratio > self.DAILY_LOSS_LIMIT:
                self.log_decision(
                    "REJECTED", f"일일 손실 한도 초과: {loss_ratio * 100:.1f}%"
                )
                return False
