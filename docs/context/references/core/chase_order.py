...
class ChaseOrderManager(QObject):
    ...
    def set_account_info(
        self, account_balance: float, yang_zhang_vol: float = 0.02
    ) -> None:
        """
        계좌 정보 설정 (포지션 사이징용)
    ...
    def _update_entry_order(self, lower_band: float, quantity: int) -> None:
        ...
        if self.ib and self.ib.isConnected():
            try:
                ...
                existing_orders = [
                    t for t in trades if t.contract.symbol == self.symbol
                ]
            except:
                pass
        ...
        if self.ib and self.ib.isConnected():
            try:
                ...
                existing_orders = [
                    t
                    for t in trades
                    if t.contract.symbol == self.symbol and t.order.action == "SELL"
                ]
            except:
                pass
    ...
    def has_active_order(self) -> bool:
        """활성 주문 존재 여부"""
        return (
            self._active_entry_order is not None or self._active_exit_order is not None
        )
