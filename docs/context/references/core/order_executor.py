...
class OrderExecutor(QObject):
    ...
    def place_market_order(
        ...
        if self.risk_manager:
            if not self.risk_manager.approve_order(
                kill_status, daily_loss, account_balance
            ):
    ...
    def place_limit_order(
        ...
        if self.risk_manager:
            if not self.risk_manager.approve_order(
                ...
                self.log_message.emit(
                    f"🚫 주문 거부됨: {action} {quantity} {symbol} @ {price}"
                )
                return None
    ...
    def _show_failure_popup(
        self, symbol: str, action: str, quantity: int, reason: str
    ) -> None:
        """
        3회 실패 시 팝업 알림
        ...
        self.log_message.emit("❌ 주문 3회 실패 - 상세 정보:")
