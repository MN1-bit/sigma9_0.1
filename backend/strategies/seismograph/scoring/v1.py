...
def calculate_score_v1(
    daily_data: Any, signal_funcs: Dict[str, Callable[[Any], float]]
) -> float:
    """
    ...
    try:
        ...
        has_accumulation_bar = (
            signal_funcs.get("accumulation_bar", lambda x: 0)(daily_data) > 0.5
        )
        has_obv_divergence = (
            signal_funcs.get("obv_divergence", lambda x: 0)(daily_data) > 0.5
        )
        has_volume_dryout = (
            signal_funcs.get("volume_dryout", lambda x: 0)(daily_data) > 0.5
        )
