...
class MassiveWebSocketClient:
    ...
    def __init__(
        ...
        logger.info(
            f"📡 MassiveWSClient initialized: {'Delayed' if delayed else 'Realtime'}"
        )
    ...
    async def unsubscribe(self, tickers: List[str], channel: Channel = Channel.AM):
        ...
        unsub_msg = json.dumps({"action": "unsubscribe", "params": params})
    ...
    def _parse_message(self, data: dict) -> Optional[dict]:
        ...
        if ev == "AM":
            ...
            # Trade (틱)
            # [08-001] event_time: 거래소 체결 시간 (Unix sec)
            # [08-001] receive_time: 서버 수신 시간 (Unix sec)
            import time as time_module

            event_time_sec = data.get("t", 0) / 1000
            receive_time_sec = time_module.time()
            ...
            tick = {
                ...
                "price": data.get("p"),
                "size": data.get("s"),
                "time": event_time_sec,  # 하위 호환성
                "event_time": event_time_sec,  # [08-001]
                "receive_time": receive_time_sec,  # [08-001]
