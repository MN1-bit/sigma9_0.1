...
try:
    ...
    logger.warning("⚠️ aiolimiter 미설치. Rate Limiting이 동작하지 않습니다. pip install aiolimiter")
...
class PolygonClient:
    ...
    Rate Limit를 준수하면서 Polygon API를 호출합니다.
    Free Tier에서는 분당 5회 호출로 제한됩니다.
    
    Attributes:
        api_key: Polygon.io API 키
        base_url: API 기본 URL
    ...
    def __init__(
        ...
        logger.debug(f"🔌 PolygonClient 초기화: rate_limit={rate_limit}/min")
            ...
            try:
                ...
                if response.status_code == 429:
                    if attempt < self.retry_count:
                        ...
                        logger.warning(f"⏳ Rate Limit 초과. {delay:.1f}초 후 재시도... ({attempt + 1}/{self.retry_count})")
                        await asyncio.sleep(delay)
                        continue
                    raise PolygonRateLimitError("Rate Limit 초과", status_code=429)
                ...
                if response.status_code >= 400:
                    ...
                    try:
                        ...
                        pass
                    raise PolygonAPIError(error_msg, status_code=response.status_code)
                ...
                if attempt < self.retry_count:
                    ...
                    logger.warning(f"🔄 네트워크 에러. {delay:.1f}초 후 재시도... ({attempt + 1}/{self.retry_count})")
                    await asyncio.sleep(delay)
                    continue
                raise PolygonAPIError(f"네트워크 에러: {e}")
        
        raise PolygonAPIError("최대 재시도 횟수 초과")
    ...
    async def fetch_grouped_daily(self, date: str) -> list[dict]:
        ...
        Polygon Grouped Daily API를 사용합니다.
    ...
    async def fetch_ticker_details(self, ticker: str) -> Optional[dict]:
        ...
        Polygon Ticker Details API를 사용합니다.
        ...
        return {
            ...
            "outstanding_shares": results.get("share_class_shares_outstanding") or results.get("weighted_shares_outstanding"),
    ...
    async def close(self) -> None:
        ...
        if self._client:
            await self._client.aclose()
            self._client = None
        logger.debug("🔌 PolygonClient 연결 종료")
