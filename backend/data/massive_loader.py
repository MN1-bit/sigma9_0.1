# ============================================================================
# Massive Data Loader - 증분 업데이트 로직
# ============================================================================
# 📌 이 파일의 역할:
#   - Massive API에서 데이터를 가져와 SQLite에 저장
...
class MassiveLoader:
    ...
    def __init__(
        ...
        parquet_manager: ParquetManager | None = None,
    ):
        """
        MassiveLoader 초기화
        ...
        Args:
            ...
            client: MassiveClient 인스턴스
            parquet_manager: ParquetManager 인스턴스 (선택, 있으면 Parquet에도 저장)
        """
        self.db = db
        self.client = client
        # Parquet 저장 (ELI5: Parquet 관리자가 있으면 Parquet 파일에도 저장합니다)
        self.parquet_manager = parquet_manager

        logger.debug(
            f"📦 MassiveLoader 초기화 (Parquet: {'✅' if parquet_manager else '❌'})"
        )
    ...
    def get_trading_days_between(start_date: datetime, end_date: datetime) -> list[str]:
        ...
        while current <= end_date:
            if MassiveLoader.is_trading_day(current):
    ...
    def get_last_trading_day() -> str:
        ...
        # 거래일 찾기
        while not MassiveLoader.is_trading_day(candidate):
    ...
    async def _save_daily_bars(self, bars: list[dict]) -> int:
        """
        일봉 데이터 저장 (SQLite + Parquet 듀얼 라이트)

        SQLite는 항상 저장하고, ParquetManager가 있으면 Parquet에도 저장합니다.
        이렇게 하면 점진적 전환이 가능합니다.

        Args:
            bars: 저장할 일봉 데이터 리스트 (dict 형태)
        ...
        # 1. SQLite 저장 (항상 실행 - 레거시 호환)
        count = await self.db.upsert_bulk(bars)

        # 2. Parquet 저장 (ParquetManager가 있을 때만)
        # ELI5: Parquet 관리자가 있으면 Parquet 파일에도 같은 데이터를 저장합니다.
        #       나중에 SQLite를 제거할 때 안전하게 전환할 수 있습니다.
        if self.parquet_manager:
            try:
                df = pd.DataFrame(bars)
                self.parquet_manager.append_daily(df)
            except Exception as e:
                # Parquet 저장 실패해도 SQLite는 성공했으므로 경고만 출력
                logger.warning(f"⚠️ Parquet 저장 실패 (SQLite는 성공): {e}")
    ...
    async def initial_load(self, days: int = 365) -> int:
        ...
        logger.info(
            f"📥 Initial Load 시작: {start_date.date()} ~ {end_date.date()} ({len(trading_days)} 거래일)"
        )
        ...
        for i, date in enumerate(trading_days):
            try:
                ...
                if bars:
                    # ─────────────────────────────────────────────────────
                    # DB 저장 (SQLite + Parquet)
                    # ─────────────────────────────────────────────────────
                    count = await self._save_daily_bars(bars)
                    total_records += count
                    success_count += 1
                ...
                if (i + 1) % 10 == 0:
                    logger.info(
                        f"📊 진행: {i + 1}/{len(trading_days)} 일 완료 ({total_records:,} 레코드)"
                    )
        ...
        logger.info(
            f"✅ Initial Load 완료: {total_records:,} 레코드 저장 (성공 {success_count}, 실패 {error_count})"
        )
    ...
    async def update_market_data(self) -> int:
        ...
        logger.info(
            f"📥 {len(missing_days)}개 거래일 업데이트 필요: {missing_days[0]} ~ {missing_days[-1]}"
        )
    ...
    async def fetch_fundamentals_batch(
        ...
        logger.info(
            f"✅ Fundamental Data 완료: {success_count}개 성공, {error_count}개 실패"
        )
...
async def create_and_update(
    ...
    Args:
        ...
        api_key: Massive API 키 (없으면 환경변수에서 가져옴)
...
if __name__ == "__main__":
    ...
    Usage:
        python massive_loader.py
    
    환경변수:
        MASSIVE_API_KEY: Massive.com API 키
    """
    import os
    import sys
