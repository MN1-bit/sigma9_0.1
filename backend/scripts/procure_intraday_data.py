...
async def procure_intraday_data(test_mode: bool = False):
    ...
    if completed:
        logger.info(f"📌 이전 진행 복원: {len(completed)}개 완료, {len(remaining)}개 남음")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test", action="store_true", help="Test mode (10 tickers only)")
