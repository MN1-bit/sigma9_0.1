...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket):
        ...
        # 연결 성공 알림
        await self._send_to_client(websocket, MessageType.STATUS, {
            "event": "connected",
            "message": "Connected to Sigma9 Trading Engine"
        })
    ...
    async def _send_to_client(
        ...
        try:
            if isinstance(data, dict):
                message = f"{msg_type.value}:{json.dumps(data)}"
            else:
    ...
    async def broadcast_tick(self, ticker: str, price: float, volume: int, timestamp: str):
        """
        틱 데이터 브로드캐스트
        ...
        Args:
            ...
            timestamp: 타임스탬프
        """
        await self.broadcast_typed(MessageType.TICK, {
            "ticker": ticker,
            "price": price,
            "volume": volume,
            "timestamp": timestamp
    ...
    async def broadcast_trade(self, event: str, order_id: str, ticker: str, **details):
        ...
        Args:
            ...
            **details: 추가 정보
        """
        await self.broadcast_typed(MessageType.TRADE, {
            "event": event,
            "order_id": order_id,
            "ticker": ticker,
            **details
    ...
    async def broadcast_watchlist(self, items: List[Dict[str, Any]]):
        """
        ...
        """
        await self.broadcast_typed(MessageType.WATCHLIST, {
            "count": len(items),
            "items": items
        })
