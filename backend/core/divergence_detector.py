...
class DivergenceDetector:
    ...
    def __init__(
        self, 
        zenV_threshold: float = 2.0, 
        zenP_threshold: float = 0.5
    ):
        """
        ...
        logger.debug(f"🔍 DivergenceDetector 초기화: zenV>={zenV_threshold}, zenP<{zenP_threshold}")
    
    def check(self, ticker: str, zenV: float, zenP: float) -> Optional[DivergenceSignal]:
        """
        Divergence 조건 확인
        ...
        if zenV >= self.ZENV_THRESHOLD and zenP < self.ZENP_THRESHOLD:
            ...
            logger.info(f"🔥 DIVERGENCE 탐지: {ticker} | zenV={zenV}, zenP={zenP}, score={signal.score}")
            return signal
