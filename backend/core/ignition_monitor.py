...
class IgnitionMonitor:
    ...
    async def start(self, watchlist: List[Dict[str, Any]]) -> bool:
        ...
        self.watchlist_data = {item.get("ticker"): item for item in watchlist if item.get("ticker")}
        ...
        logger.info(f"⚡ IgnitionMonitor 시작: {len(self.watchlist_tickers)}개 종목 모니터링 ({self.poll_interval}s 간격)")
        ...
        for ticker in self.watchlist_tickers:
            try:
                ...
                if last_close > 0:
                    ...
                    if new_score >= 50:
                        logger.debug(f"⚡ {ticker}: chg={change_pct:.1f}% base={base_score:.0f} "
                ...
                if score_delta >= 5.0 or new_score >= 50.0:
                    ...
                    passed_filter = True
                    reason = ""
                    if new_score >= 70.0 and hasattr(self.strategy, 'check_anti_trap_filter'):
    ...
    async def _fetch_quotes(self, client, tickers: List[str]) -> Dict[str, Dict[str, Any]]:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for item in data.get("tickers", []):
                    ...
                    quotes[ticker] = {
                        "price": last_trade.get("p", 0) or day.get("c", 0) or prev_day.get("c", 0),
                        "volume": day.get("v", 0),
...
def initialize_ignition_monitor(strategy: Any, ws_manager: Any, poll_interval: float = 1.0) -> IgnitionMonitor:
    """
    ...
    _monitor_instance = IgnitionMonitor(strategy, ws_manager, poll_interval)
    return _monitor_instance
