...
class TradingScheduler:
    ...
    def _setup_jobs(self):
        ...
        if self.config.market_open_scan:
            ...
            logger.info(f"📌 Job added: Market Open Scan @ {scan_hour:02d}:{scan_minute:02d} ET (Mon-Fri)")
        ...
        if self.config.daily_data_update:
            ...
            logger.info(f"📌 Job added: Daily Data Update @ {update_hour:02d}:{update_minute:02d} ET (Mon-Fri)")
    ...
    async def _run_market_open_scan(self):
        ...
        try:
            ...
            if self.db:
                ...
                logger.info(f"✅ [SCHEDULED] Market Open Scan completed: {len(result)} items")
