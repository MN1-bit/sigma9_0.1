...
class RiskManager:
    ...
    def calculate_position_size(
        ...
        if (
            self.config.use_kelly
            and len(self._trade_history) >= self.config.kelly_min_trades
        ):
            # Kelly Criterion
        ...
        logger.debug(
            f"📊 Position Size: {symbol} = {qty}주 (${position_value:,.0f} @ ${entry_price:.2f})"
        )
    ...
    def check_daily_limit(self) -> bool:
        ...
        if limit_reached:
            logger.warning(
                f"🔴 일일 손실 한도 도달: {daily_pnl_pct:.2f}% (한도: {self.config.daily_loss_limit_pct}%)"
            )
    ...
    def check_weekly_limit(self) -> bool:
        ...
        if limit_reached:
            logger.warning(
                f"🔴 주간 손실 한도 도달: {weekly_pnl:.2f}% (한도: {self.config.weekly_loss_limit_pct}%)"
            )
    ...
    def is_trading_allowed(self) -> bool:
        ...
        if today in self._daily_pnl:
            if self._daily_pnl[today].trade_count >= self.config.max_daily_trades:
                logger.debug(
                    f"🚫 거래 불가: 일일 거래 한도 ({self.config.max_daily_trades}회)"
                )
                return False
        ...
        try:
            ...
            for pos in positions:
                ...
                if qty > 0:
                    ...
                    result["liquidated_positions"].append(
                        {"symbol": symbol, "qty": qty, "action": "SELL"}
                    )
                    ...
                    result["liquidated_positions"].append(
                        {"symbol": symbol, "qty": abs(qty), "action": "BUY"}
                    )
                    logger.info(f"📤 청산: BUY {abs(qty)} {symbol}")

            logger.warning(
                f"✅ Kill Switch 완료: {len(result['liquidated_positions'])}개 포지션 청산"
            )
    ...
    def enable_trading(self) -> None:
        ...
        if self._is_killed:
            logger.warning(
                "⚠️ Kill Switch가 발동 중입니다. 먼저 reset_kill_switch()를 호출하세요."
            )
            return
        self._trading_enabled = True
        logger.info("🟢 거래 활성화")
