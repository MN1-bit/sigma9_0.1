...
class BacktestEngine:
    ...
    def __init__(
        ...
        logger.debug(f"🔬 BacktestEngine 초기화 (DB: {db_path})")
    ...
    async def _load_all_data(
        ...
        for ticker in tickers:
            # DB에서 데이터 조회
            bars = await self.db.get_daily_bars(ticker, days=500)
    ...
    async def _check_entries(
        ...
        for candidate in candidates[:available_slots]:
            ...
            logger.debug(f"🟢 진입: {ticker} @ ${entry_price:.2f} (Stage {candidate['stage']}, Score {candidate['score']:.0f})")
    ...
    async def _check_exits(
        ...
        for ticker, exit_price, exit_reason, exit_date in to_close:
            ...
            # 현금 복구
            entry_value = self.config.initial_capital * (self.config.position_size_pct / 100)
            exit_value = entry_value * (1 + pnl / 100)
            self._cash += exit_value
            
            emoji = "🟢" if pnl > 0 else "🔴"
            logger.debug(f"{emoji} 청산: {ticker} @ ${exit_price:.2f} ({exit_reason}, P&L {pnl:+.2f}%)")
    ...
    def _update_equity(
        ...
        for ticker, trade in self._open_positions.items():
            ...
            if not current_bar.empty:
                ...
                position_value = (self.config.initial_capital * self.config.position_size_pct / 100) * (1 + pnl_pct / 100)
...
async def main():
    ...
    parser.add_argument("--tickers", nargs="+", default=["AAPL"], help="Tickers to backtest")
