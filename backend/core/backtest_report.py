...
class BacktestReport:
    ...
    def profit_factor(self) -> float:
        ...
        """
        total_wins = sum(
            t.pnl_pct or 0 for t in self.get_closed_trades() if t.is_winner
        )
        total_losses = abs(
            sum(t.pnl_pct or 0 for t in self.get_closed_trades() if not t.is_winner)
        )
    ...
    def print_summary(self) -> None:
        ...
        print(
            f"🏆 Avg Win: {summary['avg_win_pct']:+.2f}% | 📉 Avg Loss: {summary['avg_loss_pct']:.2f}%"
        )
